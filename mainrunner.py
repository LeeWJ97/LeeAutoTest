...
if 1:
    ...
    from common.xpathmap import XpathMap




    logger.info(f'Run, Current Path is: {sysKey.path}')
    ...
    # 获取分组显示
    tr = '<tr><td width="35" height="28" align="center" bgcolor="#FFFFFF" style="border:1px solid #ccc;">Auto Type</td><td width="70" height="28" align="center" bgcolor="#FFFFFF" style="border:1px solid #ccc;">Case Name</td><td width="100" height="28" align="center" bgcolor="#FFFFFF" style="border:1px solid #ccc;">Group Info</td><td width="50" height="28" align="center" bgcolor="#FFFFFF" style="border:1px solid #ccc;">Total Cases</td><td width="50" align="center" bgcolor="#FFFFFF" style="border:1px solid #ccc;">Pass Cases</td><td width="50" align="center" bgcolor="#FFFFFF" style="border:1px solid #ccc;">Test Status</td></tr>'
    trs = ""

    for i in range(len(groups)):
        try:
            tmp = tr.replace('Auto Type', str(groups[i][0]))
            tmp = tmp.replace('Case Name',str(groups[i][1]))
            tmp = tmp.replace('Group Info', str(groups[i][2]))
            tmp = tmp.replace('Total Cases', str(groups[i][3]))
            tmp = tmp.replace('Pass Cases', str(groups[i][4]))
            tmp = tmp.replace('Test Status', str(groups[i][5]))
        except Exception as e:
            logger.error(str(e))
        trs += tmp

    html = html.replace('mailbody',trs)
    mail = Mail([f'./lib/{casename}_result.xlsx'],[f'{casename}_TestResult.xlsx'])
    mail.send(html)
    time.sleep(5)